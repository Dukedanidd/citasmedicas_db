...
-- Dump completed on 2025-05-08 15:16:48

DELIMITER ;;
CREATE TRIGGER trg_usuarios_ai_medico
AFTER INSERT ON usuarios
FOR EACH ROW
BEGIN
    IF NEW.role_id = 2 THEN
        -- Verificar si ya existe una entrada en medicos
        IF NOT EXISTS (SELECT 1 FROM medicos WHERE doctor_id = NEW.user_id) THEN
            SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Los usuarios con rol de médico deben tener una entrada en la tabla medicos';
        END IF;
    END IF;
END;;
DELIMITER ;
